...
from SvenBot.models import SlackNotification, SlackNotificationType, SlackEvent, SlackCalendarEvent, ResponseData, \
    Embed, EmbedField
from SvenBot.utility import CHANNELS_URL, ARCHUB_URL, ARCHUB_HEADERS
...
def test_main_event(httpx_mock: HTTPXMock):
    ...
    httpx_mock.add_response(
        method="GET",
        url=f"{ARCHUB_URL}/operations/next",
        status_code=HTTP_200_OK,
    ...
    assert httpx_mock.get_request(method="POST").content.decode() == ResponseData(
        ...
        embeds = [Embed(
            ...
            fields=[
                EmbedField(name="Start", value=f"<t:{startTime}:t>", inline=True),
                EmbedField(name="End", value=f"<t:{endTime}:t>", inline=True),
                EmbedField(name=mission1.display_name, value=f"[Co-op by {mission1.maker}](https://arcomm.co.uk/hub/missions/{mission1.id})", inline=False),
                EmbedField(name=mission2.display_name, value=f"[TvT by {mission2.maker}](https://arcomm.co.uk/hub/missions/{mission2.id})", inline=False),
                EmbedField(name=mission3.display_name, value=f"[ARCade by {mission3.maker}](https://arcomm.co.uk/hub/missions/{mission3.id})", inline=False),
            ],
